...
func (a *assignmentApi[ParentEntity, ChildEntity]) AssignRoute(middleware ...fiber.Handler) routing.Route {
	...
	return routing.Route{OpenAPIMetadata: routing.OpenAPIMetadata{
		...
		Description: fmt.Sprintf(
			"This endpoint assigns a %s to a %s",
		...
		if !reflect.ValueOf(existingAssociation).IsZero() {
			return ctx.Status(fiber.StatusBadRequest).
				JSON(fiber.Map{
					"error": "Bad Request",
					"message": fmt.Sprintf(
						"The %s is already assigned to the %s.",
						strings.ToLower(a.childName),
						strings.ToLower(a.parentName),
					),
				})
